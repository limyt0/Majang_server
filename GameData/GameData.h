...
public:
  ...
  //vector<Jaksadata> jaksas; //참가 작사들. 들어간 순서대로 차례임
  int Oyaindex = 0; // 현재 동이 누구인지
  Baram nowTableBaram = Baram.Dong;//현재 장풍
  vector<int> peasan; //패산 정보
  vector<int> wangpae; //도라표시패, 도라표시패아래, 영상패.(고정)
  vector<int> doras_omote; //앞 도라들 열릴때마다 추가
  vector<int> doras_ura;//뒷도라들 열릴때 마다 추가
...
{
public:
    vector<int> berimBeas; //버린패들
    vector<int> behurod;//내가 버린거 남이 후로해간거. 버린패에서 제외필요.
